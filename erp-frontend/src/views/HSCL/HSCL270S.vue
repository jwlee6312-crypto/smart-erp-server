<!--
	...
	작성일자	: 2025.02.24
	설명        : 담당자별 당월/합계 매출 총이익 현황 조회 (표준 UI 적용)
	=============================================================
-->
...
const search = async () => {
	...
	try {
		const res = await api.post('/hscl/HSCL_270S_STR', {
...
const print = () => {
	...
	window.open(`/hscl/HSCL_270P?${params}`, 'Print', 'width=1000,height=800')
}
	...
	if (mainGridRef.value) {
		mainGrid = new Tabulator(mainGridRef.value, {
			...
			columns: [
				...
				{
					title: "영업담당명", field: "usernm", minWidth: 150, hozAlign: "left",
				...
				},
				{
					title: "당월실적",
					columns: [
						{ title: "매출금액", field: "salsamt", hozAlign: "right", width: 130, formatter: "money", formatterParams: { precision: 0 }, bottomCalc: "sum", bottomCalcFormatter: "money" },
						...
						{
							title: "이익률(%)", field: "prof_rate", hozAlign: "right", width: 80,
							formatter: (cell) => {
								const d = cell.getData()
				...
				},
				{
					title: "합계(누계)",
					columns: [
						{ title: "매출금액", field: "salsamt_t", hozAlign: "right", width: 140, formatter: "money", formatterParams: { precision: 0 }, bottomCalc: "sum", bottomCalcFormatter: "money" },
						...
						{
							title: "이익률(%)", field: "prof_rate_t", hozAlign: "right", width: 80,
							formatter: (cell) => {
								const d = cell.getData()

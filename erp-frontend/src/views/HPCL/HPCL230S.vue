  ...
  <div class="erp-container">
    ...
    <div class="erp-header d-flex justify-content-between align-items-center border-bottom bg-white py-2 px-3 sticky-top shadow-sm">
      <div class="fw-bold text-dark d-flex align-items-center" style="font-size: 14px;">
        <i class="bi bi-layers-half me-2 text-primary" style="font-size: 18px;"></i>
        생산정보 <i class="bi bi-chevron-right mx-2 small opacity-50"></i>
        <span class="text-primary fw-bolder">공정재공수불부 (HPCL230S)</span>
      </div>
    ...
    <div class="flex-grow-1 overflow-hidden p-2 d-flex flex-column gap-2">
      ...
      <div class="card border-0 shadow-sm flex-grow-1 overflow-hidden d-flex flex-column" style="border-radius: 8px;">
        <div class="card-header bg-white py-2 px-3 border-bottom d-flex justify-content-between align-items-center">
          <span class="fw-bold small text-dark"><i class="bi bi-table me-1 text-primary"></i> 공정별 재공품 상세 수불</span>
        </div>
...
const fetchList = async () => {
  ...
  try {
    const res = await api.post('/hpcl/HPCL_230S_STR', {
...
const exportExcel = () => {
  grid?.download("xlsx", `공정재공수불부_${searchData.yy}${searchData.mm}.xlsx`, { title: "공정재공 수불부" })
}
...
const formatDateString = (v: any, sep: string) => v && String(v).length >= 6 ? `${v.substring(0, 4)}${sep}${v.substring(4, 6)}${sep}${v.substring(6, 8)}` : (v || '')

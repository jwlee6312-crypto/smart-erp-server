<!--
	=============================================================
	프로그램명: 미지급금 잔액등록 (HABA240U)
	작성일자	: 2025.03.14
	작성자    : AI Assistant
	설명        : 계정과목 및 거래처별 미지급금 기초 잔액 관리 (HSOD100U 균등 배분 레이아웃 적용)
	=============================================================
-->
		...
		<div class="flex-grow-1 overflow-hidden p-2 d-flex flex-column gap-2 bg-light main-content-wrapper">
			...
			<div class="card border shadow-sm flex-shrink-0 overflow-hidden">
				<div class="card-header bg-white py-1 px-3 border-bottom d-flex align-items-center">
					...
					<span class="fw-bold small text-dark">미지급금 상세 정보 [{{ masterForm.actkind === 'I1' ? '신규등록' : '정보수정' }}]</span>
				</div>
				<div class="card-body p-0 bg-white">
					<table class="erp-table-dense w-100">
						<colgroup>
						...
						<tbody>
							<tr>
								...
								<td>
									<input v-model="masterForm.slipymd" type="date" class="form-control form-control-sm" style="max-width: 140px;" />
								</td>
								<th class="required bg-light small text-center border-start">미지급금</th>
								<td>
									<div class="d-flex align-items-center gap-1 px-1">
										<input v-model="masterForm.Upayamt" type="number" class="form-control form-control-sm text-end fw-bold text-primary" style="max-width: 160px;" />
										<span class="small text-muted">원</span>
									</div>
								</td>
							</tr>
							<tr>
								<th class="bg-light small text-center border-top">지불조건</th>
								<td class="border-top">
									<select v-model="masterForm.paycndt" class="form-select form-select-sm" style="max-width: 160px;">
			...
			<div class="card border shadow-sm flex-grow-1 d-flex flex-column overflow-hidden bg-white">
				<div class="card-header bg-white py-1 px-3 border-bottom d-flex align-items-center justify-content-between flex-shrink-0">
					<span class="fw-bold small text-dark"><i class="bi bi-grid-3x3-gap-fill me-2 text-primary"></i>미지급금 잔액 목록</span>
				</div>
...
// 🚀 [방식 A] 거래처 팝업은 SaleCustHelp 컴포넌트 사용
...
onMounted(() => {
	...
	if (mainGridRef.value) {
		mainGrid = new Tabulator(mainGridRef.value, {
			...
			columns: [
				{ title: "거래처코드", field: "col5", width: 110, hozAlign: "center", cssClass: "fw-bold text-primary" },
				{ title: "거래처명", field: "col6", minWidth: 200, hozAlign: "left", cssClass: "fw-bold" },
				{ title: "발생일", field: "col0", width: 110, hozAlign: "center", formatter: (cell) => formatDate(cell.getValue()) },
				{ title: "미지급액", field: "col7", width: 130, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } },

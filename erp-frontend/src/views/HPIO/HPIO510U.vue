...
async function fetchDetail(row: any) {
  ...
  try {
    const res = await api.post('/hpio/HPIO_511U_STR', { actkind: 'S', cmpycd: authStore.cmpycd, iogbn: '200', ioym: row.ioym, iono: row.iono });
...
async function saveData() {
  ...
  try {
    ...
    const resM = await api.post('/hpio/HPIO_510U_STR', { ...masterData, actkind, cmpycd: authStore.cmpycd, iogbn: '200', userid: authStore.userid, cfmyn: 'Y' });
    ...
    for (const item of details) {
      ...
      await api.post('/hpio/HPIO_511U_STR', {

<!--
	...
	작성일자	: 2025.03.23 (Applied - HASL010U Consistency Standard)
	설명        : 재무 부서용 회계 전표 관리 (고밀도 무결성 버전)
	=============================================================
-->
...
<template>
  ...
  <HelpBase ref="bugtHelpRef" v-model:visible="popVisible.bugt" title="예산과목 선택" :columns="bugtPopupColumns" @search="fetchBugtPopupData" @confirm="onBugtConfirm" @close="restoreFocus" />
              ...
              <table class="erp-table-dense w-100">
                ...
                <tbody>
                  <tr>
                    ...
                    <th class="bg-light text-center small border-start">회계일자</th>
                    <td><input v-model="masterForm.acctymd" type="date" class="form-control" tabindex="5" /></td>
                  </tr>
                  <tr>
                    ...
                    <th class="bg-light text-center border-top small border-start">상태</th>
                    <td class="border-top text-center">
                       <span :class="masterForm.slipgu === '020' ? 'badge bg-success' : 'badge bg-primary'">{{ masterForm.slipgu === '020' ? '미확정' : '확정' }}</span>
                    </td>
                  </tr>
                </tbody>
              </table>
                 ...
                 <table class="erp-table-dense side-detail-table w-100 border-0 bg-white">
                    ...
                    <tbody>
                        ...
                        <template v-else>
                            ...
                            <tr v-if="String(selectedRowData.typebugt || '').trim() > '000'">
                                <th class="bg-light text-end pe-3">예산과목</th>
                                <td><div class="input-group input-group-sm"><input v-model="selectedRowData.bugtnm" class="form-control border-0" readonly placeholder="검색(Enter)" tabindex="1006" @keydown.enter="handleOpenHelp('BUGT')" @keydown.tab="handleDetailExit" /><button class="btn btn-outline-secondary border-0 px-2" @click="handleOpenHelp('BUGT')" tabindex="-1"><i class="bi bi-search"></i></button></div></td>
                            </tr>
                        </template>
                    </tbody>
                 </table>
...
const bugtPopupColumns = [{ title: '코드', field: 'bugtcd', width: 100 }, { title: '예산과목명', field: 'bugtnm', widthGrow: 1 }]
...
const acctHelpRef = ref<any>(null); const prjHelpRef = ref<any>(null); const mgtHelpRef = ref<any>(null); const bugtHelpRef = ref<any>(null)

const popVisible = reactive({ dept: false, cust: false, acct: false, prj: false, mgt: false, sub: false, vat_cust: false, bugt: false })
...
const masterForm = reactive<any>({ cmpycd: authStore.cmpycd, deptcd: authStore.deptcd, deptnm: authStore.deptnm, slipymd: today, slipno: '', empnm: authStore.usernm, business: '', acctymd: today, slipgu: '020' })
...
const initialize = () => {
    ...
    totalDebit.value = 0; totalCredit.value = 0; Object.assign(masterForm, { cmpycd: authStore.cmpycd, deptcd: authStore.deptcd, deptnm: authStore.deptnm, slipymd: today, acctymd: today, empnm: authStore.usernm, slipgu: '020', business: '' });
    if (grid2) {
        grid2.setData([]);
...
const onBugtConfirm = (d: any) => {
    if (!activeRowComponent) return;
    const rowData = activeRowComponent.getData();
    rowData.bugtcd = d.bugtcd || ''; rowData.bugtnm = d.bugtnm || '';
    activeRowComponent.update(rowData);
    selectedRowData.value = activeRowComponent.getData();
    handleDetailAutoNext();
}
...
async function searchSlips() { const res = await api.post('/hasl/HASL_110U_STR', { actkind: 'F', cmpycd: authStore.cmpycd, fromdt: searchParams.fromdt.replace(/-/g, ''), todt: searchParams.todt.replace(/-/g, ''), keyword: searchParams.business }); grid1?.setData(res.data || []); vAlert('조회되었습니다(Alt+F)') }
async function fetchDetail(row: any) {
    ...
    try {
      ...
      const data = (resd.data || []).map((i:any) => ({
        ...
        subnm: i.custnm || '', subcd: i.custcd || '', docno9nm: i.docno9nm || i.docno9 || ''
      }));
...
const fetchBugtPopupData = async (word: string) => { const rowData = selectedRowData.value; if (!rowData) return; api.post('/ha00/HA00_00P_STR', { gubun: 'B0', cmpycd: authStore.cmpycd, gbncd: '', remark: rowData.acctcd, code: word || '' }).then(r => bugtHelpRef.value?.setData(r.data)) }
...
const handleOpenHelp = (type: string, target?: any) => {
  ...
  if (type === 'DEPT') popVisible.dept = true
  ...
  else if (type === 'PRJ' || type === 'MGT' || type === 'VAT_CUST' || type === 'SUB' || type === 'BUGT') {
    ...
    else if (type === 'BUGT') popVisible.bugt = true
...
const handleFullDelete = async () => {
    ...
    try {
        const payload = { actkind: 'D', master: { ...masterForm, slipymd: masterForm.slipymd.replace(/-/g, '') }, details: [] };
        const res = await api.post('/hasl/HASL_110U_SAVE', payload);
...
onMounted(() => {
  ...
  grid1 = new Tabulator(slipListGridRef.value!, {
    ...
    columns: [
        ...
        { title: "전표금액", field: "slipamt", width: 100, hozAlign: "right", formatter: "money", headerSort: false }
    ]
  });

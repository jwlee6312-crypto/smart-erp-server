<!--
	...
	작성일자	: 2025.02.24
	설명        : 일반발주 마스터/상세 관리 (HSOD100U 표준 규격 적용)
	=============================================================
-->
              ...
              <table class="erp-table-dense w-100">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <select v-model="form_02.bal_userid" class="form-select" :disabled="isClosed" tabindex="9">
...
async function save() {
  ...
  try {
    ...
    await api.post('/hsio/HSIO_052U_SAVE', { mst, dtl: details })
...
async function handleFullDelete() {
  ...
  if (confirm('정말 삭제하시겠습니까?')) {
    try {
      await api.post('/hsio/HSIO_052U_SAVE', {
        mst: { actkind: 'D', cmpycd: authStore.cmpycd, balym: form_02.balym, balno: form_02.balno, updemp: authStore.userid },
        dtl: []
      })
...
const generateOrderHtml = () => {
    ...
    dtl.forEach(i => {
        ...
        rowsHtml += `<tr height="30"><td align="left">${i.itemnm}</td><td align="right">${fC(qty)}</td><td align="right">${fC(qty > 0 ? amt/qty : 0)}</td><td align="right">${fC(amt)}</td><td align="right">${fC(vat)}</td><td align="right">${fC(amt+vat)}</td><td></td></tr>`;
    });
    return `<html><body onload="window.print()"><h2>일반발주서</h2><table border="1">${rowsHtml}</table></body></html>`;
};
...
function handleGlobalShortcuts(e: KeyboardEvent) {
  if (e.altKey) {
    ...
    else if (key === 'h') { e.preventDefault(); manualStore.open('HSIO052U') }
  }
}
...
onMounted(async () => {
  grid1 = new Tabulator(tableRef1.value!, {
    ...
    columns: [
      ...
      { title: "발주번호", field: "balno_full", width: 140, cssClass: "fw-bold text-primary", mutator: (v,d)=>`${d.balym}-${d.iono || d.balno}` }
    ]
  })

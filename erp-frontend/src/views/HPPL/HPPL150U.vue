<!--
	...
	작성일자	: 2025.03.14
	설명        : 양산 생산 요청 자료 등록 및 관리 (HSOD100U 무결성 표준 적용본)
	=============================================================
-->
    ...
    <div class="flex-grow-1 overflow-hidden p-2 d-flex flex-column gap-2 bg-light main-content-wrapper">
      ...
      <div class="card border shadow-sm flex-grow-1 overflow-hidden d-flex flex-column grid-container-right">
        <div class="card-header bg-white py-1 px-3 border-bottom d-flex align-items-center justify-content-between flex-shrink-0">
          <span class="fw-bold small text-dark d-flex align-items-center"><i class="bi bi-grid-3x3-gap-fill me-2 text-primary"></i>양산 생산 요청 내역</span>
...
const onItemConfirm = (d: any) => {
  ...
  setTimeout(() => activeRow.getCell("planqty").edit(), 150)
}
...
const handleRemarkTab = (e: KeyboardEvent) => {
...
const handleGlobalShortcuts = (e: KeyboardEvent) => {
  if (e.altKey) {
    ...
    else if (key === 'h') { e.preventDefault(); manualStore.open('HPPL150U') }
  }
}
  ...
  nextTick(() => {
    ...
    mainGrid = new Tabulator(mainGridRef.value, {
      ...
      columns: [
        ...
        { title: '요청일자', field: 'reqymd', width: 110, hozAlign: 'center', editor: 'date',
          formatter: (c) => {
            const v = c.getValue(); return v && v.length === 8 ? `${v.substring(0, 4)}-${v.substring(4, 6)}-${v.substring(6, 8)}` : v;
          },
          cellEdited: (cell) => {
            const row = cell.getRow(); const ymd = String(cell.getValue()).replace(/-/g, '')
            if (ymd.length >= 6) row.update({ reqym: ymd.substring(0, 6) })
            updateRowStatus(row)
          }
        },
        { title: '관리번호', field: 'reqno_full', width: 130, hozAlign: 'center', cssClass: 'fw-bold text-primary',
        ...
        { title: '요청수량', field: 'planqty', width: 90, hozAlign: 'right', editor: 'number', formatter: 'money', formatterParams: { precision: 0 }, cellEdited: (cell) => updateRowStatus(cell.getRow()) },
        ...
        { title: '거래처', field: 'custnm', widthGrow: 1, cellClick: (e, cell) => handleOpenHelp('CUST', cell.getRow()) },
        { title: '비고', field: 'bigo', widthGrow: 1, editor: 'input', cellEdited: (cell) => updateRowStatus(cell.getRow()) },

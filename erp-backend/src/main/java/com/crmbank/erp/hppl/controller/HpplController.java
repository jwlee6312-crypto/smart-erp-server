...
/**
 * [HPPL] 생산계획 통합 컨트롤러 (사용자 정의 최종 표준형)
 */
...
public class HpplController {
    ...
    @PostMapping("/HPPL_100U_STR")
    public ResponseEntity<?> callHPPL_100U_STR(@RequestBody Map<String, Object> params, HttpSession session) {
        injectSession(params, session);
        params.putIfAbsent("yymmdd", "");
        fillMissingParameters("HPPL_100U_STR", params);
        log.info("🏢 [Master SQL]: {}", buildPositionalSql("HPPL_100U_STR", params));

        String actkind = String.valueOf(params.getOrDefault("actkind", "S0")).toUpperCase();
        List<Map<String, Object>> raw = hpplMapper.HPPL_100U_STR(params);
    ...
    @PostMapping("/HPPL_120U_STR")
    public ResponseEntity<?> callHPPL_120U_STR(@RequestBody Map<String, Object> params, HttpSession session) {
        injectSession(params, session);
        fillMissingParameters("HPPL_120U_STR", params);
        log.info("🏢 [Master SQL]: {}", buildPositionalSql("HPPL_120U_STR", params));

        String actkind = String.valueOf(params.getOrDefault("actkind", "S")).toUpperCase();
        List<Map<String, Object>> raw = hpplMapper.HPPL_120U_STR(params);
    ...
    @PostMapping("/HPPL_150U_STR")
    public ResponseEntity<?> callHPPL_150U_STR(@RequestBody Map<String, Object> params, HttpSession session) {
        injectSession(params, session);
        fillMissingParameters("HPPL_150U_STR", params);
        log.info("🏢 [Master SQL]: {}", buildPositionalSql("HPPL_150U_STR", params));

        String actkind = String.valueOf(params.getOrDefault("actkind", "S")).toUpperCase();
        List<Map<String, Object>> raw = hpplMapper.HPPL_150U_STR(params);
    ...
    @PostMapping("/HPPL_110S_STR")
    public ResponseEntity<?> callHPPL_110S_STR(@RequestBody Map<String, Object> params, HttpSession session) {
        injectSession(params, session);
        Object ymd = params.getOrDefault("yymmdd", params.getOrDefault("iyymmdd", params.getOrDefault("iyymmDD", "")));
        params.put("yymmdd", ymd);
        fillMissingParameters("HPPL_110S_STR", params);
        log.info("🏢 [Exec SQL]: {}", buildPositionalSql("HPPL_110S_STR", params));
        return ResponseEntity.ok(convertToLowerCaseKeys(hpplMapper.HPPL_110S_STR(params)));
    }

...
/**
 * 🚀 [MHSIO080S] 발주현황
 * 웹 HSIO080S.vue 로직 기반 모바일 최적화 버전
 */
public class MHSIO080S extends BaseActivity {

    private TextView tvDateFrom, tvDateTo, tvDeptNm, tvCount, tvTotalAmt;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mhsio080s);
    ...
    private void search() {
        if (deptcd.isEmpty()) { Toast.makeText(this, "발주부서를 선택하세요.", Toast.LENGTH_SHORT).show(); return; }
        ...
        apiService.executeHsioProcedure("HSIO_080S_STR", p).enqueue(new Callback<List<Map<String, Object>>>() {
            @Override public void onResponse(@NonNull Call<List<Map<String, Object>>> call, @NonNull Response<List<Map<String, Object>>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    updateTotals();
                    if (dataList.isEmpty()) Toast.makeText(MHSIO080S.this, "조회된 내역이 없습니다.", Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void updateTotals() {
        ...
        for (Map<String, Object> item : dataList) {
            sum += getDoubleVal(item, "balamt") + getDoubleVal(item, "balvat");
        }
    ...
    @Override protected String getProgramTitle() { return "발주현황"; }
    @Override protected String getProgramId() { return "MHSIO080S"; }
    ...
    private class OrderAdapter extends BaseAdapter {
        ...
        @Override public View getView(int p, View v, ViewGroup pr) {
            if (v == null) v = LayoutInflater.from(MHSIO080S.this).inflate(R.layout.item_mhsio080s, pr, false);
            ...
            int cnt = (int) getDoubleVal(item, "item_cnt");
            if (cnt > 0) itemNm += " 외 " + cnt + "건";
            ((TextView) v.findViewById(R.id.tvItemNm)).setText(itemNm);

            ((TextView) v.findViewById(R.id.tvQty)).setText(df.format(getDoubleVal(item, "balqty")));
            
            double total = getDoubleVal(item, "balamt") + getDoubleVal(item, "balvat");
